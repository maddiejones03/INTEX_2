...
-- Matches: Appendix A Data Dictionary exactly
-- ============================================================
...
-- ============================================================
-- USERS  (auth — not in data dictionary but required for portal)
-- ============================================================
...
CREATE TABLE safehouses (
    ...
    region             VARCHAR(50)  NOT NULL
                                    CHECK (region IN ('Luzon', 'Visayas', 'Mindanao')),
    ...
    capacity_girls     INT          NOT NULL CHECK (capacity_girls > 0),
    capacity_staff     INT,
...
-- ============================================================
-- SOCIAL MEDIA POSTS
-- (created before donations because donations.referral_post_id → here)
-- ============================================================
...
CREATE TABLE social_media_posts (
    ...
    post_hour                     SMALLINT        CHECK (post_hour BETWEEN 0 AND 23),
    ...
    call_to_action_type           VARCHAR(20)
                                                  CHECK (call_to_action_type IN (
                                                      'DonateNow', 'LearnMore', 'ShareStory', 'SignUp'
                                                  )),
...
CREATE TABLE donations (
    ...
    impact_unit            VARCHAR(20)
                                           CHECK (impact_unit IN ('pesos', 'items', 'hours', 'campaigns')),
    is_recurring           BOOLEAN         NOT NULL DEFAULT FALSE,
    campaign_name          VARCHAR(200),
    notes                  TEXT,
    created_by_partner_id  INT             REFERENCES partners(partner_id) ON DELETE SET NULL,
...
CREATE TABLE in_kind_donation_items (
    ...
    item_category          VARCHAR(30)     NOT NULL
                                           CHECK (item_category IN (
                                               'Food', 'Supplies', 'Clothing', 'SchoolMaterials',
                                               'Hygiene', 'Furniture', 'Medical'
                                           )),
    quantity               INT             NOT NULL CHECK (quantity > 0),
    unit_of_measure        VARCHAR(20)     NOT NULL
                                           CHECK (unit_of_measure IN ('pcs', 'boxes', 'kg', 'sets', 'packs')),
    ...
    received_condition     VARCHAR(20)
                                           CHECK (received_condition IN ('New', 'Good', 'Fair'))
);
...
CREATE TABLE donation_allocations (
    ...
    program_area       VARCHAR(30)     NOT NULL
                                       CHECK (program_area IN (
                                           'Education', 'Wellbeing', 'Operations',
                                           'Transport', 'Maintenance', 'Outreach'
                                       )),
...
CREATE TABLE residents (
    ...
    birth_status              VARCHAR(20)
                                           CHECK (birth_status IN ('Marital', 'Non-Marital')),
    ...
    referral_source           VARCHAR(30)
                                           CHECK (referral_source IN (
                                               'Government Agency', 'NGO', 'Police',
                                               'Self-Referral', 'Community', 'Court Order'
                                           )),
    ...
    -- Reintegration
    reintegration_type        VARCHAR(40)
                                           CHECK (reintegration_type IN (
...
CREATE TABLE process_recordings (
    ...
    emotional_state_observed  VARCHAR(20) NOT NULL
                                          CHECK (emotional_state_observed IN (
                                              'Calm', 'Anxious', 'Sad', 'Angry',
                                              'Hopeful', 'Withdrawn', 'Happy', 'Distressed'
                                          )),
    emotional_state_end       VARCHAR(20)
                                          CHECK (emotional_state_end IN (
                                              'Calm', 'Anxious', 'Sad', 'Angry',
                                              'Hopeful', 'Withdrawn', 'Happy', 'Distressed'
                                          )),
...
CREATE TABLE education_records (
    ...
    program_name         VARCHAR(30)     NOT NULL
                                         CHECK (program_name IN (
                                             'Bridge Program', 'Secondary Support',
                                             'Vocational Skills', 'Literacy Boost'
                                         )),
    course_name          VARCHAR(30)     NOT NULL
                                         CHECK (course_name IN (
                                             'Math', 'English', 'Science', 'Life Skills',
                                             'Computer Basics', 'Livelihood'
                                         )),
    ...
    gpa_like_score       NUMERIC(3, 1)   CHECK (gpa_like_score BETWEEN 1.0 AND 5.0),
    notes                TEXT
);
...
CREATE TABLE health_wellbeing_records (
    ...
    bmi                        NUMERIC(4, 2),
    nutrition_score            NUMERIC(3, 1)  CHECK (nutrition_score BETWEEN 1.0 AND 5.0),
    sleep_score                NUMERIC(3, 1)  CHECK (sleep_score BETWEEN 1.0 AND 5.0),
    energy_score               NUMERIC(3, 1)  CHECK (energy_score BETWEEN 1.0 AND 5.0),
    general_health_score       NUMERIC(3, 1)  CHECK (general_health_score BETWEEN 1.0 AND 5.0),
...
CREATE INDEX idx_donations_campaign      ON donations(campaign_name);
...
CREATE INDEX idx_education_date          ON education_records(record_date);
...
CREATE INDEX idx_posts_campaign          ON social_media_posts(campaign_name);

-- partner assignments
CREATE INDEX idx_assignments_partner     ON partner_assignments(partner_id);
CREATE INDEX idx_assignments_safehouse   ON partner_assignments(safehouse_id);

-- safehouse monthly metrics
CREATE INDEX idx_metrics_safehouse       ON safehouse_monthly_metrics(safehouse_id);
CREATE INDEX idx_metrics_month           ON safehouse_monthly_metrics(month_start);

-- ============================================================
-- VIEWS
-- ============================================================

-- Live donor totals per supporter (no stored column needed)
...
-- Live occupancy check (safehouses.current_occupancy is stored but this view
-- lets you cross-verify against actual active residents)
